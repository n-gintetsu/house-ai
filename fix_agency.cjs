...
// AgencyForm.jsx を作成
const agencyForm = `import { useState } from 'react'
...
fs.writeFileSync('src/AgencyForm.jsx', agencyForm, 'utf8');
console.log('SUCCESS: src/AgencyForm.jsx を作成しました');

// App.jsx に AgencyForm を組み込む
let app = fs.readFileSync('src/App.jsx', 'utf8');

// import追加
if (!app.includes("import AgencyForm from './AgencyForm'")) {
  app = app.replace(
    "import AuthPanel from './AuthPanel'",
    "import AuthPanel from './AuthPanel'\nimport AgencyForm from './AgencyForm'"
  );
  console.log('AgencyForm import を追加しました');
}

// agencyタブの中身を差し替え
const agencyStart = app.indexOf("{tab === 'agency'");
if (agencyStart === -1) {
  console.log('ERROR: agency タブが見つかりませんでした');
  process.exit(1);
}
...
const newAgencyBlock = `{tab === 'agency' && (
            <div className="ha-panel" style={{ padding: 0 }}>
              <AgencyForm />
            </div>
          )}`;

app = app.substring(0, agencyStart) + newAgencyBlock + app.substring(pos);
fs.writeFileSync('src/App.jsx', app, 'utf8');
console.log('SUCCESS: App.jsx の業者タブを AgencyForm に差し替えました');
