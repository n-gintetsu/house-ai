...
import {
  Calculator, TrendingUp, GraduationCap, FileText,
  Receipt, BookOpen, Shield, Home, Sparkles, ArrowLeft,
} from 'lucide-react';
...
export default function ToolHubPage({ onSelectTool, onBack }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F9FC', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', maxWidth: '1080px', margin: '0 auto', padding: '40px 20px 100px' }}>
        ...
        <motion.button
          ...
          style={{
            ...
            color: '#4B5563', fontSize: '14px', fontWeight: 600,
            marginBottom: '40px', padding: '8px 0',
          }}
        >
          <ArrowLeft size={18} />
          戻る
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center', marginBottom: '56px' }}>
          ...
          <h2 style={{
            fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 700, marginBottom: '12px',
            background: 'linear-gradient(to right, #111827, #1e3a5f, #111827)',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
          }}>
            便利ツール
          </h2>
          <p style={{ color: '#4B5563', fontSize: '18px', margin: 0 }}>不動産の悩みをAIと整理できます</p>
        </motion.div>

        {/* Tool Grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }} className="thp-grid">
          {tools.map((tool, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Title */}
                <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#111827', marginBottom: '8px', margin: '0 0 8px' }}>
                  {tool.title}
                </h3>

                {/* Description */}
                <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '16px', lineHeight: 1.6, margin: '0 0 16px' }}>
                  {tool.description}
                </p>
      ...
      {loadingTool ? (
        ...
        >
          <motion.div
            ...
            style={{ background: 'white', borderRadius: '28px', padding: '48px', maxWidth: '400px', width: '100%', textAlign: 'center' }}
          >
            <motion.div
              style={{
            ...
            <div style={{ height: '4px', background: '#F3F4F6', borderRadius: '999px', overflow: 'hidden' }}>
              <motion.div
                style={{ height: '100%', background: 'linear-gradient(to right, #3b82f6, #9333ea)', borderRadius: '999px' }}
      ...
      {/* Responsive grid breakpoints — media queries require a style tag */}
      <style>{`
        @media (max-width: 900px) { .thp-grid { grid-template-columns: repeat(2, 1fr) !important; } }
        @media (max-width: 600px) { .thp-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </div>
  );
}
