    ...
    <div style={{ minHeight: '100vh', background: '#0A1628', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ position: 'absolute', inset: 0 }}>
        {[0,1,2,3,4,5,6,7].map((i) => (
          <div key={i} style={{ position: 'absolute', bottom: 0, left: `${i * 12}%`, width: '10%', height: `${40 + i * 5}%`, background: 'linear-gradient(to top, #1a2744, transparent)', borderTop: '2px solid rgba(0,212,255,0.3)' }} />
        ))}
        {[0,1,2,3,4].map((i) => (
          <div key={i} style={{ position: 'absolute', left: `${20 + i * 15}%`, top: 0, width: 2, height: '100%', background: 'linear-gradient(to bottom, transparent, #00D4FF, transparent)' }} />
        ))}
      </div>
      <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '0 16px', maxWidth: 900 }}>
        ...
        <h1 style={{ fontSize: '3rem', fontWeight: 700, marginBottom: 24, background: 'linear-gradient(to right, #D4AF37, #FFD700, #D4AF37)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          投稿完了
        </h1>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16 }}>
            {actions.map((action, index) => (
              <button key={action.label} onClick={() => navigate(action.path)} style={{ padding: 24, borderRadius: 16, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer' }}>
