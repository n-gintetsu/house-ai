  ...
  async function loadProperties() {
    ...
    const normalizedAgency = (agencyProps || []).map(p => {
      ...
      return {
        ...
        built_date: details.built_date || null,
        structure: details.structure || null,
        floor_info: details.floor_info || null,
        management_fee: details.management_fee || null,
        repair_fund: details.repair_fund || null,
        parking: details.parking || null,
        delivery: details.delivery || null,
        transaction_type: details.transaction_type || null,
        land_rights: details.land_rights || null,
        use_zone: details.use_zone || null,
        remarks: details.remarks || null,
        current_status: details.current_status || null,
        deposit: details.deposit || null,
        key_money: details.key_money || null,
    ...
    <div style={{ padding: 20 }}>
        ...
        <>
          ...
          {searched && (
            <>
              ...
              {matched.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 16px', color: '#777', fontSize: 14 }}>条件に合う物件が見つかりませんでした。</div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
                  {matched.map(p => <PropertyCard key={p.id} property={p} isFavorite={favorites.includes(p.id)} onToggleFavorite={toggleFavorite} onClick={() => setSelectedProperty(p)} />)}
                </div>
              )}
            </>
          )}
          ...
          {!searched && properties.length > 0 && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
                {properties.map(p => <PropertyCard key={p.id} property={p} isFavorite={favorites.includes(p.id)} onToggleFavorite={toggleFavorite} onClick={() => setSelectedProperty(p)} />)}
              </div>
            </>
          )}
      ...
      {activeTab === 'favorites' && (
        <div>
          ...
          {favProps.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 16px', color: '#777', fontSize: 14 }}>お気に入りに登録した物件はありません。</div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
              {favProps.map(p => <PropertyCard key={p.id} property={p} isFavorite={true} onToggleFavorite={toggleFavorite} onClick={() => setSelectedProperty(p)} />)}
            </div>
          )}
        </div>
      )}
    ...
    >
      <div style={{ width: '100%', aspectRatio: '4/3', background: '#f0f4f8', overflow: 'hidden', position: 'relative' }}>
        {p.image_url
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#bbb', fontSize: 36 }}>🏠</div>
        }
        <button onClick={e => onToggleFavorite(p.id, e)} style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {isFavorite ? '❤️' : '🤍'}
        </button>
        <div style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(26,58,92,0.7)', color: '#fff', fontSize: 10, padding: '3px 7px', borderRadius: 4 }}>
          詳細を見る →
        </div>
      </div>
      <div style={{ padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          {p.deal_type && <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: p.deal_type === 'sale' ? '#fef3c7' : '#dbeafe', color: p.deal_type === 'sale' ? '#92400e' : '#1e40af', fontWeight: 700 }}>
            {p.deal_type === 'sale' ? '売買' : p.deal_type === 'both' ? '売買・賃貸' : '賃貸'}
          </span>}
...
function PropertyModal({ property: p, isFavorite, onToggleFavorite, onClose }) {
  ...
  const rows = [
    { label: '所在地', value: p.address },
    { label: '交通・アクセス', value: p.access },
    ...
    { label: '面積', value: p.area ? \`\${p.area}㎡\` : null },
    { label: '物件種別', value: p.property_type },
    { label: '築年月', value: p.built_date },
    { label: '構造', value: p.structure },
    { label: '階数/所在階', value: p.floor_info },
    { label: '管理費', value: p.management_fee },
    { label: '修繕積立金', value: p.repair_fund },
    { label: '敷金', value: p.deposit },
    { label: '礼金', value: p.key_money },
    { label: '契約期間', value: p.contract_period },
    { label: '駐車場', value: p.parking },
    { label: '引渡', value: p.delivery },
    { label: '取引態様', value: p.transaction_type },
    { label: '土地権利', value: p.land_rights },
    { label: '用途地域', value: p.use_zone },
    { label: '現況', value: p.current_status },
    { label: '備考', value: p.remarks },
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 600, position: 'relative', marginTop: 20, marginBottom: 20 }}>
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
            ...
            {p.deal_type && <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 4, background: p.deal_type === 'sale' ? '#fef3c7' : '#dbeafe', color: p.deal_type === 'sale' ? '#92400e' : '#1e40af', fontWeight: 700 }}>
              {p.deal_type === 'sale' ? '売買' : p.deal_type === 'both' ? '売買・賃貸' : '賃貸'}
            </span>}
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
            ...
            {p.address && (
              <a href={searchUrl} target="_blank" rel="noreferrer" style={{ flex: 1, padding: '10px', border: '1.5px solid #1a3a5c', borderRadius: 10, background: '#1a3a5c', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', textDecoration: 'none', textAlign: 'center' }}>
                📍 Google Mapsで開く
              </a>
            )}
          </div>
...
console.log('SUCCESS: PropertyMatching.jsx に詳細モーダルを追加しました！')
