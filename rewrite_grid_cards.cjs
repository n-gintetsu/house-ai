...
export default function PropertyMatching({ user }) {
  ...
  async function loadProperties() {
    ...
    let agencyQuery = supabase.from('agency_properties').select('*').eq('status', 'active').eq('is_public', true)
        ...
        <>
          ...
          {searched && (
            <>
              ...
              {matched.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 16px', color: '#777', fontSize: 14 }}>条件に合う物件が見つかりませんでした。<br />条件を変えて再検索してください。</div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
                  {matched.map(p => <PropertyCard key={p.id} property={p} isFavorite={favorites.includes(p.id)} onToggleFavorite={toggleFavorite} />)}
                </div>
              )}
            </>
          )}
          ...
          {!searched && properties.length > 0 && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
                {properties.map(p => <PropertyCard key={p.id} property={p} isFavorite={favorites.includes(p.id)} onToggleFavorite={toggleFavorite} />)}
              </div>
            </>
          )}
      ...
      {activeTab === 'favorites' && (
        <div>
          ...
          {favProps.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
              {favProps.map(p => <PropertyCard key={p.id} property={p} isFavorite={true} onToggleFavorite={toggleFavorite} />)}
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #f5a623', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ width: '100%', aspectRatio: '4/3', background: '#f0f4f8', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* お気に入りボタン */}
        <button onClick={() => onToggleFavorite(p.id)} style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {isFavorite ? '❤️' : '🤍'}
        </button>
      </div>
      ...
      <div style={{ padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        {/* 地図ボタン */}
        {p.address && (
          <a href={searchUrl} target="_blank" rel="noreferrer" style={{ marginTop: 6, fontSize: 11, padding: '5px 10px', borderRadius: 8, border: '1px solid rgba(26,58,92,0.2)', background: '#f8fafc', color: '#555', textDecoration: 'none', textAlign: 'center' }}>
            📍 Google Mapsで開く
          </a>
        )}
      </div>
